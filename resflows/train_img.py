...
import ellipses_dataset2 as ellipses_ds
...
elif args.data == 'ellipses':
    im_dim = 1
    init_layer = layers.LogitTransform(0.05)
    #space = uniform_discr(min_pt=[-1., -1.], max_pt=[1., 1.], shape=(args.imagesize, args.imagesize), dtype=np.float32)
    #geometry = parallel_beam_geometry(space, num_angles=30)
    #ray_trafo = RayTransform(space, geometry, impl='astra_cuda')
    ellipses_gen = ellipses_ds.ellipses_generator(size = args.imagesize)
    ellipses_dataset = ellipses_ds.GeneratorTorchDataset(ellipses_gen, 32000, shape=(1, args.imagesize, args.imagesize))
    train_loader = torch.utils.data.DataLoader(ellipses_dataset, batch_size=args.batchsize, num_workers=args.nworkers)
    test_loader = torch.utils.data.DataLoader(ellipses_dataset, batch_size=args.val_batchsize, num_workers=args.nworkers)
elif args.data == 'lodopab':
    im_dim = 1
    init_layer = layers.LogitTransform(0.05)
    lodopab_dival_dataset = lodopab_ds.LoDoPaBSmallDataset(size=args.imagesize, angle_step=20, detector_pixel_step=2)
    num_angles = lodopab_dival_dataset.shape[0][0]
    lodopab_dataset = lodopab_dival_dataset.create_torch_dataset(part='train', reshape=((1, num_angles, -1), (1, args.imagesize, args.imagesize)))
    train_loader = torch.utils.data.DataLoader(lodopab_dataset, batch_size=args.batchsize, num_workers=args.nworkers)
    lodopab_testset = lodopab_dival_dataset.create_torch_dataset(part='test', reshape=((1, num_angles, -1), (1, args.imagesize, args.imagesize)))
    test_loader = torch.utils.data.DataLoader(lodopab_testset, batch_size=args.batchsize, num_workers=args.nworkers)
...
def validate(epoch, model, ema=None):
    ...
    start = time.time()
    with torch.no_grad():
        for i, (y, x) in enumerate(tqdm(test_loader)):
            x = x.to(device)
...
def visualize(epoch, model, itr, real_imgs):
    ...
    with torch.no_grad():
        ...
        save_image(imgs.cpu().float(), filename, nrow=args.batchsize, padding=2)
    model.train()
